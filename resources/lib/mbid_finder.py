...
class MBIDFinder:
    ...
    def find(self):
        ...
        settings.log("MBIDFinder: %s - %s" % (self.artist_name, self.album_name))
...
class TadbMBIDFinder(MBIDFinder):
    ...
    def find(self):
        settings.log("TadbMBIDFinder: %s - %s" % (self.artist_name, self.album_name))
        result = MBIDResult()
        if self.artist_name is not None:
            ...
            try:
                ...
                if "artists" in json and json["artists"] is not None:
                    ...
                    if "strMusicBrainzID" in bestmatch and result.is_mbid(bestmatch["strMusicBrainzID"]):
                ...
                if "album" in json and json["album"] is not None:
                    ...
                    if "strMusicBrainzArtistID" in bestmatch and result.is_mbid(bestmatch["strMusicBrainzArtistID"]):
...
class MusicbrainzMBIDFinder(MBIDFinder):
    ...
    def find(self):
        ...
        settings.log("MusicbrainzMBIDFinder: %s - %s" % (self.artist_name, self.album_name))
        result = MBIDResult()
        if self.artist_name is not None:

            # @TODO: rewrite musicbrainz collector
            if self.artist_name == "AC/DC" or self.artist_name == "Kygo feat. Conrad":
                ...
                if artist_element is not None:
                    settings.log("QuickX: %s" % artist_element.get("id", None))
...
class ThrottledMusicbrainzRequest:
    """Throttles a Musicbrainz request to avoid getting banned"""
    __last_request = 0
...
class MBIDResult:
    ...
    def has_artist_mbid(self):
        return self.is_mbid(self.artist_mbid)
    ...
    def has_album_mbid(self):
        return self.is_mbid(self.album_mbid)
